...
def extract_text_from_pdf(env, pdf_hash):
    ...
    body = {"base64Source": base64_encoded_pdf}

    # Call Azure Document Intelligence API with retry logic
    max_retries = 3
    retry_count = 0
    ...
    while retry_count < max_retries:
        try:
            ...
            if response.status_code == 202:  # Accepted - async operation
                ...
                return poll_for_result(operation_location, key)
            elif response.status_code == 401:  # Unauthorized - token might have expired
                logging.warning(
                    "Authentication failed, attempting to refresh credentials"
                )
                # Refresh credentials could be implemented here if using token-based auth
                retry_count += 1
            else:
                logging.error(
            ...
            logging.error(f"Error calling Document Intelligence API: {str(e)}")
            retry_count += 1

    logging.error("Failed to extract text after multiple retries")
    return None


def poll_for_result(operation_location, key):
    """Poll for the async operation result with exponential backoff"""
    ...
    while status == "running" and retries < max_retries:
        try:
            ...
            if result_response.status_code != 200:
                logging.error(
                    f"Error polling for result: {result_response.status_code} - {result_response.text}"
                )
                return None
            ...
            if status == "succeeded":
                ...
                logging.error(
                    f"Document processing failed: {result.get('error', {}).get('message', 'Unknown error')}"
                )
                return None
        except Exception as e:
    ...
    logging.error("Document processing timed out")
    return None
    ...
    for i, (idx, row) in enumerate(df.iterrows()):
        try:
            ...
            if i % 10 == 0:
                ...
                logging.info(
                    f"Processed {i}/{len(df)} items for {manufacturer} {product_type}"
                )
