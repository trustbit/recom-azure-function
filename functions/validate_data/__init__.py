...
from shared.model import PowerConverterList
...
def validate_data_for_manufacturer(env, manufacturer, product_type):
    ...
    for hash_str in pdf_hashes:
        ...
        if html_file in all_files and pdf_file in all_files:
            ...
            logging.warning(
                f"For hash {hash_str} files are missing: html={html_file in all_files}, pdf={pdf_file in all_files}"
            )
            continue
        ...
        try:
            ...
            # Create series entry
            serie = {"pdf": pdf_rows.model_dump(), "html": html_rows, "hash": hash_str}
            series.append(serie)
    ...
    logging.info(
        f"Validated {len(series)} groups with {total_records} products and saved to merged.json"
    )
