...
from shared.models import PowerConverterList, PowerConverterModel
...
def group_product_fields(records):
    ...
    ingest_data = {
        'common_series_info': common_fields,
        'products': records
    }
...
def extract_document_content(env, pdf_content, json_data, count):
    ...
    response = client.beta.chat.completions.parse(
        ...
        messages=messages,
        response_format=PowerConverterList
    )
...
def extract_structured_data_for_manufacturer(env, manufacturer, product_type):
    ...
    for pdf_hash, group in pdf_groups:
        try:
            ...
            subset_df = group[['Mounting Type','Certifications','Isolation (kV)', 
                              'type','series','part_number', 'power', 'isolation', 
                              'vin', 'main_vout', 'package_style']].copy()
            ...
            structured_data = extract_with_caching(env, current_full_step, latex_content, json_data, group_count)
...
def main(params) -> dict:
    ...
    try:
        ...
        # Extract structured data
        result = extract_structured_data_for_manufacturer(env, manufacturer, product_type)
