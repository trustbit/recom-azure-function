...
async def scrape_recom_products(env, product_type):
    ...
    async with async_playwright() as p:
        ...
        for _, series in series_df.iterrows():
            try:
                series_url = "https://recom-power.com/en/products" + series["product_link"].strip(".")
                ...
                logging.error(f"Error scraping series {series.get('product_name', 'unknown')}: {str(e)}")
                continue
    ...
    for row in table.find_all('tr')[1:]:
        try:
            ...
            if 'part_number' in column_indices:
                product['part_number'] = cols[column_indices['part_number']].find('span', {'class': 'part-number'}).text.strip()
            ...
            if 'main_vout_(v)' in column_indices:
                product['main_vout'] = cols[column_indices['main_vout_(v)']].text.strip()
            if 'iout_1_(ma)' in column_indices:
                product['iout1'] = cols[column_indices['iout_1_(ma)']].text.strip()
            if 'package_style' in column_indices:
                product['package_style'] = cols[column_indices['package_style']].text.strip()
            ...
            for col in cols:
                ...
                if image_div:
                    break
            product['image_url'] = image_div.find('a', {'class': 'rbutton'})['href'] if image_div else None
    ...
    async with async_playwright() as p:
        ...
        for _, series in series_df.iterrows():
            try:
                ...
                # Get series features
                features = soup.select_one('div.field--name-field-features div.field__items')
                series_features = features.text.strip() if features else ""
                
                # Get product links
                product_links = soup.select("div.content article.model a.model-page-link")
                ...
                for link in product_links:
                    try:
                        ...
                        # Basic info
                        part_number = product_soup.select_one("div.field--name-title h1")
                        product["part_number"] = part_number.text.strip() if part_number else ""
                        
                        description = product_soup.select_one("div.field--name-field-description")
                        product["description"] = description.text.strip() if description else ""
                        
                        # Datasheet link
                        datasheet_link = product_soup.find('a', string='Datasheet')
                        product["datasheet_link"] = datasheet_link['href'] if datasheet_link else series["datasheet_link"]
                        
                        # Attributes
                        attribute_items = product_soup.select("div.model-attributes-container div.attribute-item")
                        ...
                        logging.warning(f"Error processing product {link.get('href', 'unknown')}: {str(e)}")
                        continue
                
                # Add delay between series
                await asyncio.sleep(2)
            except Exception as e:
                logging.error(f"Error scraping series {series.get('product_series', 'unknown')}: {str(e)}")
                continue
    ...
    async with async_playwright() as p:
        ...
        for _, series in series_df.iterrows():
            try:
                ...
                await page.wait_for_selector("table.min-w-full", state="visible", timeout=10000)
                
                # Get series description
                description = {}
                description_div = await page.query_selector('div.single-product .fadeup.fadeup-2')
                if description_div:
                    ...
                    description["subtitle"] = subtitle_elem.text.strip() if subtitle_elem else ""
                ...
                if features_header:
                    features_div = await features_header.evaluate('el => el.closest("div.fadeup-group")')
                    if features_div:
                        features_html = await page.evaluate('div => div.outerHTML', features_div)
                ...
                for row in product_rows:
                    try:
                        ...
                        datasheet_elem = row.find(lambda tag: tag.name == 'a' and 'Datasheet' in tag.text)
                ...
                logging.error(f"Error scraping series {series.get('productCode', 'unknown')}: {str(e)}")
                continue
