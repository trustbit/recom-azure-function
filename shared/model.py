...
class PowerDerating(BaseModel):
    ...
    unit: Literal["W", "%", "%/K", "%/C"] = Field(
        ...,
        description="Unit of output power derating - percent, Watt or rate (% per C/K)",
    )
    rate: float = Field(..., description="Value of power derating at the temperature")


# DO NOT CHANGE ANYTHING. READ FILE HEADER FIRST
MountingTypes = Literal[
    "PCB Mount", "DIN-Rail", "Chassis Mount", "Panel Mount", "Wired", "Other"
]
ConnectionTypes = Literal["THT", "SMD", "BGA", "Wire", "Terminal", "Other"]
ComponentStyle = Literal[
    "DIP", "QFP", "SOIC", "QFN", "LGA", "SOP", "SIP", "LOC", "Other"
]
...
class ComponentPackage(BaseModel):
    package_name: Optional[str] = Field(
        ..., description="Component package name, e.g. SIP-7, LGA-24, Half-Brick etc"
    )
    mounting_type: Optional[MountingTypes] = Field(
        None, description="How is the component installed"
    )
    connection_type: Optional[ConnectionTypes] = Field(
        None, description="How is component connected electrically"
    )
    ...
    brick_size: Optional[
        Literal["Half", "Quarter", "Eighth", "Sixteenth", "Thirty-second", "other"]
    ] = None
...
PinType = Literal[
    ...
    "-Sense",
    "Trim",
    "CTRL/UVLO (Under Voltage Lock Out)",
    "Other",
...
class PowerConverterModel(BaseModel):
    product_series: Optional[str] = Field(
        None,
        description="Product series, examples: RxxPxx/R, RAC02E-K/277, RxxC05TExxS, RFMM, RPMD15-FW",
    )
    part_number: str = Field(
        ...,
        description="Part/Model number. Pay attention to model numbering rules. Examples: RAC04-12SK/277, RACM65-24S-ST, REM2A-0515S, RP30-2415SFW/N-HC",
    )
    converter_type: ConverterTypes = Field(
        ..., description="Converter type (DC/DC, AC/DC)"
    )

    ac_voltage_input_min: Optional[float] = Field(
        None, description="Minimum input AC voltage"
    )
    ac_voltage_input_max: Optional[float] = Field(
        None, description="Maximum input AC voltage"
    )
    dc_voltage_input_min: Optional[float] = Field(
        None, description="Minimum input DC voltage"
    )
    dc_voltage_input_max: Optional[float] = Field(
        None, description="Maximum input DC voltage"
    )

    input_voltage_tolerance: Optional[float] = Field(
        None, description="Typical input voltage tolerance in percent. E.g. typ. ±5%"
    )

    power: Optional[float] = Field(None, description="Power in watts")

    is_regulated: Optional[bool] = Field(
        None, description="Is this converter regulated?"
    )
    regulation_voltage_range: Optional[str] = Field(
        None, description="Input voltage range. E.g. 10:1"
    )
    ...
    # from the pin table
    output_type: Optional[OutputTypes] = Field(
        None, description="Output type (Single, Dual, Triple, etc.)"
    )
    # pins need output type
    pins: List[Pin] = Field(
        ..., description="Pin configuration. Pay attention to component output type"
    )
    package: Optional[ComponentPackage] = Field(
        None, description="Component package information"
    )

    packaging_type: Optional[PackagingType] = Field(
        None,
        description="Packaging type (e.g., Tube, Tape & Reel, Tray, Cardboard Box, Single Box, Airbag, Moisture Barrier Bag, Other)",
    )

    dimensions: Optional[Dimensions] = Field(
        None, description="Dimensions in mm: length, width, height"
    )
    certifications: Optional[List[str]] = Field(
        None, description="List of certifications"
    )
    protections: Optional[List[str]] = Field(
        None, description="List of protection features"
    )

    # table or chart-based usually
    operating_temperature: Optional[TemperatureRange] = Field(
    ...
    power_derating: List[PowerDerating] = Field(
        ..., description="How does temperature affect output power?"
    )


PowerConverterModel_schema = json.dumps(
    PowerConverterModel.model_json_schema(), indent=2, ensure_ascii=False
)
