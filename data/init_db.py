...
def create_tables():
    ...
    cursor.execute(
        "CREATE INDEX IF NOT EXISTS idx_product_series ON converters(product_series)"
    )
    cursor.execute(
        "CREATE INDEX IF NOT EXISTS idx_part_number ON converters(part_number)"
    )
    cursor.execute(
        "CREATE INDEX IF NOT EXISTS idx_converter_type ON converters(converter_type)"
    )
    cursor.execute("CREATE INDEX IF NOT EXISTS idx_power ON converters(power)")
    cursor.execute(
        "CREATE INDEX IF NOT EXISTS idx_efficiency ON converters(efficiency)"
    )
    cursor.execute(
        "CREATE INDEX IF NOT EXISTS idx_operating_temp_min ON converters(operating_temp_min)"
    )
    cursor.execute(
        "CREATE INDEX IF NOT EXISTS idx_operating_temp_max ON converters(operating_temp_max)"
    )
