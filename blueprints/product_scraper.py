...
def scrape_products(input: dict) -> dict:
    ...
    logging.info(f"Scraping products for {input['manufacturer']} {input['product_type']}")
            ...
            try:
                ...
                for row in rows:
                    try:
                        ...
                        # Get datasheet link
                        datasheet_cell = await row.query_selector("td.additional-column")
                        datasheet_link = ""
                        if datasheet_cell:
                ...
                logging.info(f"Scraped {len(series_products)} products from {series['product_name']}")
            ...
            try:
                ...
                for row in rows:
                    try:
                        ...
                        part_number_text = part_number.text.strip() if part_number else ""
                        ...
                        if datasheets:
                            datasheet_link = "https://www.tracopower.com" + datasheets[0]["href"]
                        ...
                        product = {
                            ...
                            "datasheet_link": datasheet_link,
                            "series_power_variants": series.get("series_power_variants", ""),
                            "series_input_voltage_variants": series.get("series_input_voltage_variants", ""),
                            "series_output_voltage_variants": series.get("series_output_voltage_variants", ""),
                            "series_description": series.get("series_short_description", ""),
                        }
                ...
                all_products.extend(series_products)
                logging.info(f"Scraped {len(series_products)} products from {series.get('product_series', 'unknown')}")
                
            except Exception as e:
                logging.error(f"Error scraping Traco series {series.get('product_series', 'unknown')}: {str(e)}")
            finally:
                await page.close()
...
async def scrape_xppower_products(env, product_type):
    ...
    async with async_playwright() as p:
        ...
        for _, series in series_df.iterrows():
            ...
            try:
                # Navigate to series page
                url = "https://www.xppower.com" + series["url"] if not series["url"].startswith("http") else series["url"]
                await page.goto(url, wait_until="networkidle")
                ...
                for row in rows:
                    try:
                        ...
                        logging.warning(f"Error processing XP Power product row: {str(e)}")
                        continue
                
                all_products.extend(series_products)
                logging.info(f"Scraped {len(series_products)} products from {series.get('productCode', 'unknown')}")
                
            except Exception as e:
                logging.error(f"Error scraping XP Power series {series.get('productCode', 'unknown')}: {str(e)}")
            finally:
                await page.close()
