...
def download_pdfs(input: dict) -> dict:
    ...
    logging.info(f"Downloading PDFs for {input['manufacturer']} {input['product_type']}")
    ...
    try:
        ...
        products_df = products_df[products_df["datasheet_link"].notna() & (products_df["datasheet_link"] != "")]
        ...
        for index, product in products_df.iterrows():
            try:
                ...
                # Generate filename
                product_code = product["product_code"].replace("/", "_").replace("\\", "_")
                ...
                if response.status_code == 200:
                    ...
                    env.storage.save_blob(container_name, f"{product_type}/{pdf_filename}", response.content)
                    downloaded += 1
                else:
                    logging.warning(f"Failed to download PDF for {product_code}: HTTP {response.status_code}")
                    failures += 1
            except Exception as e:
                logging.warning(f"Error downloading PDF for {product.get('product_code', 'unknown')}: {str(e)}")
                failures += 1
...
def extract_pdf_data(input: dict) -> dict:
    ...
    logging.info(f"Extracting PDF data for {input['manufacturer']} {input['product_type']}")
            ...
            try:
                ...
                try:
                    ...
                    try:
                        ...
                        logging.warning(f"Both PDF extraction methods failed for {filename}: {str(e2)}")
                        continue
